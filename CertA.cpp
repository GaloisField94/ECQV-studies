...
uint16_t p_bits;
ZZ p, n, a, b, Gx, Gy, tmpX, tmpY, dCA, k, e, r;
ECpoint *theGenerator, *QCA, *RU, *kG, *PU, *condition;
FILE *fp;
...
int main(int argc, char const *argv[]) {
  ...
  readFile.open("CAprivateFile.txt");
  if(!readFile.is_open()) {
    std::cout << "Couldn't open desired file!" << "\n";
    return 0;
  }
  std::getline(readFile, tmp_str);
  dCA = conv<ZZ>(tmp_str.c_str());
  readFile.close();
  //ECpoint *QCA2 = theCurve->mulPoint(theGenerator, dCA);
  //QCA2->printlnPoint(); checked, it's correct

  //receive request
  umask(0);
  mknod("mkfifoRUx", S_IFIFO|0666, 0);
  std::cout << "CA ready to receive a new request" << "\n";
  fp = fopen("mkfifoRUx", "r");
  buffer1 = (char *)malloc(sizeof *buffer1 * (MAX_ZZ_BYTES + 1));
  it = 0;
  while((tmp_c = getc(fp)) != EOF) buffer1[it++] = tmp_c;
  //std::cout << "received" << buffer1 << "\n";
  ZZFromBytes(tmpX, (unsigned char*)buffer1, MAX_ZZ_BYTES);
  //std::cout << "Received " << tmpX << "\n";
  fclose(fp);
  unlink("mkfifoRUx");
  umask(0);
  mknod("mkfifoRUy", S_IFIFO|0666, 0);
  fp = fopen("mkfifoRUy", "r");
  buffer2 = (char *)malloc(sizeof *buffer1 * (MAX_ZZ_BYTES + 1));
  it = 0;
  while((tmp_c = getc(fp)) != EOF) buffer2[it++] = tmp_c;
  //std::cout << "received" << buffer2 << "\n";
  ZZFromBytes(tmpY, (unsigned char*)buffer2, MAX_ZZ_BYTES);
  //std::cout << "Received " << tmpY << "\n";
  fclose(fp);
  unlink("mkfifoRUy");

  //after receiving request
  RU = new ECpoint(tmpX, tmpY);
  std::cout << "Received request RU = ";
  RU->printlnPoint();
  bool kIsBad = false;
  unsigned char *PUx_str = (unsigned char *)malloc(sizeof *PUx_str * MAX_ZZ_BYTES);
  unsigned char *PUy_str = (unsigned char *)malloc(sizeof *PUy_str * MAX_ZZ_BYTES);
  unsigned char *CertU = (unsigned char *)malloc(sizeof *CertU * MAX_ZZ_BYTES * 2);
  unsigned char *e_str = (unsigned char *)malloc(sizeof *e_str * gcry_md_get_algo_dlen(chosenHashFunction));
  do {
    k = RandomBnd(n);
    kG = theCurve->mulPoint(theGenerator, k);
    //PU=RU+kG
    PU = theCurve->addPoints(RU, kG);
    //CertU=Enc(PU)
    BytesFromZZ(PUx_str, PU->getX(), MAX_ZZ_BYTES);
    BytesFromZZ(PUy_str, PU->getY(), MAX_ZZ_BYTES);
    for(int i = 0; i < MAX_ZZ_BYTES; i++) {
      CertU[i] = PUx_str[i];
      CertU[i+MAX_ZZ_BYTES] = PUy_str[i];
    }
    //e=Hn(CertU)
    gcry_md_hash_buffer(chosenHashFunction, e_str, CertU, MAX_ZZ_BYTES * 2);
    ...
    //if ePU+QCA=inf find better k
    condition = theCurve->addPoints(theCurve->mulPoint(PU, e), QCA);
    if(condition->isInfinite()) kIsBad = true;
  } while(kIsBad);
  AddMod(r, MulMod(e, k, n), dCA, n);
  //send (r,CertU)
  unsigned char *r_str = (unsigned char *)malloc(sizeof *r_str * MAX_ZZ_BYTES);
  BytesFromZZ(r_str, r, MAX_ZZ_BYTES);

  fp = fopen("mkfifor", "w");
  fputs((char *)r_str, fp);
  fclose(fp);
  fp = fopen("mkfifoCertU", "w");
  fputs((char *)CertU, fp);
  fclose(fp);
  std::cout << "Calculated e=Hn(CertU), e = " << e << "\n";
  std::cout << "Calculated PU and encoded it into CertU, PU = ";
  PU->printlnPoint();
  std::cout << "Sent response (r, CertU), where r = " << r << "\n";
